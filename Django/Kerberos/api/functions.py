...
def get_scheduls(input_data:dict):
    ...
    kerubero_id = int(input_data["kerbero_id"])

    val = {"schedules":[]}

    for v in  Schedule.objects.filter(kerubero_id = kerubero_id,date__range=[appear_date,end_date]):
...
def make_schedules(new_rest_request:RestRequest):
    rest_requests = RestRequest.objects.filter(kerbero_id=new_rest_request.kerbero_id,head_name=new_rest_request.head_name,date=new_rest_request.date)
    ...
    if ("left" in name_set) and ("center" in name_set) and ("right" in name_set):
        ...
        for i, val in enumerate(rest_requests):
            ...
            if i == 0:
                setting_val.append({"head_name":current_head,"start_time":current_start_datetime,"end_time":current_end_datetime+timedelta(hours=6)})
                ...
                if (current_start_datetime <= (setting_val[0]["start_time"] - timedelta(hours=6)) and current_start_datetime <= (setting_val[1]["start_time"] - timedelta(hours=6))) or (current_start_datetime <= (setting_val[0]["start_time"] - timedelta(hours=6)) and current_start_datetime >= (setting_val[1]["end_time"])) or (current_start_datetime <= (setting_val[1]["start_time"] - timedelta(hours=6)) and current_start_datetime >= (setting_val[0]["end_time"])) or (current_start_datetime >= (setting_val[0]["end_time"]) and current_start_datetime <= (setting_val[1]["end_time"])):
                    ...
                    setting_val.append({"head_name":current_head,"start_time":current_check_val,"end_time":current_check_val + timedelta(hurs=6)})
        ...
        for val in setting_val:
            if val["head_name"] == "left":
                ...
                left_val["end_time"] = make_end_time_str(date_of,val["end_time"])

            elif val["head_name"] == "center":
                center_val["start_time"] = val["start_time"].strftime('%H:%M')
                center_val["end_time"] = make_end_time_str(date_of,val["end_time"])
            
            elif val["head_name"] == "right":
                right_val["start_time"] = val["start_time"].strftime('%H:%M')
                right_val["end_time"] = make_end_time_str(date_of,val["end_time"])
...
def make_end_time_str(date,end_time):
    if date == end_time.date:
        return end_time.strftime('%H:%M')

    else:
        return str(end_time.hour + 24) + ":" + str(end_time.minute)